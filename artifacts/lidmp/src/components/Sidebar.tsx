...
import { Globe, Map, Bot, Rocket, BarChart3, AlertTriangle, Cpu, ChevronLeft, ChevronRight, Satellite, Activity } from "lucide-react";
    ...
    >
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => onNav(item.id)} data-testid={`nav-${item.id}`}
              ...
              {!collapsed && (
                <>
                  ...
                  {item.badge && (
                    <span className={`pill text-[8px] ${item.badgeStyle ?? "pill-slate"}`}>{item.badge}</span>
                  )}
                </>
              )}
            </button>
